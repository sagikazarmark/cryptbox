...
/// A [`SweepStore`] over one `PostgreSQL` table with an integer cursor column.
///
/// Available with both `migrate` and `sqlx-postgres` features.
///
...
/// directly.
pub struct PostgresSweepStore<'a> {
    connection: &'a mut PgConnection,
    sql: SweepSql,
}

impl<'a> PostgresSweepStore<'a> {
    /// Creates a store over a connection and table description.
    #[must_use]
    pub fn new(connection: &'a mut PgConnection, table: &SweepTable) -> Self {
        Self {
            connection,
            sql: table.sql(ParamStyle::Dollar),
        }
    }
