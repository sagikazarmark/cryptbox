...
This application's provider is a startup snapshot. Before promoting generation 2,
distribute both generations to **all readers**, verify readiness, then restart
writers with `CRYPTBOX_GENERATION=2`. The old ciphertext is not rewritten merely
by promotion. Use the [key rotation example](../examples/key_rotation.rs) and
[maintenance sweep guide](reencryption-sweep.md) for rewriting existing data;
keep old index generations probeable until their tokens have been replaced.
Retain historical encryption/index pairs for backups even after live convergence.
Fleet rollout/recovery guidance continues in [#58](https://github.com/sagikazarmark/cryptbox/issues/58).
